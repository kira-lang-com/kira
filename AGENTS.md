...
Landing strategy: small direct commits to `main` are fine, but never fast-forward a long stack of individual commits onto `main` — above all the upstream repo's `main`. When landing a branch (or any multi-commit unit of work), use a no-fast-forward merge (`git merge --no-ff`, or the GitHub "Create a merge commit" option) so `main`'s first-parent history records one merge commit per landed unit; the branch's individual commits stay in the second-parent DAG, off `main`'s first-parent line. Do not `git branch -f main <branch>` + push a fast-forward of many commits, do not "Squash and merge", and do not "Rebase and merge" onto `main`. The upstream `main` should read as merge commits per landed unit, not a long list of individual commits. Relatedly, never run `git reset --hard` or any operation that discards uncommitted working-tree changes — repo worktrees routinely carry the user's uncommitted WIP, and a hard reset destroys it irreversibly; use `git reset --soft` or `git stash` (never `drop`) and ask if unsure.
