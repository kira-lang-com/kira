...
- `examples/sokol_triangle/main.kira`

There is also `examples/sokol_runtime_entry/main.kira`, which uses the same general binding flow with slightly different state handling.
...
```bash
zig build run -- run --backend hybrid tests/pass/run/ffi_sokol_hybrid/main.kira
```
